...
(function(){
  ['ax_gantt.js','ax_tabs.js'].forEach(function(f){
...
(function(){
  ...
  function styles(){
    ...
    #v-outputs .grp{display:inline-flex;gap:0}
    #v-outputs .grp .bs{border-radius:0}
    #v-outputs .grp .bs:first-child{border-radius:5px 0 0 5px}
    #v-outputs .grp .bs:last-child{border-radius:0 5px 5px 0;border-left:none}
    #v-outputs .grp .bs.mini{padding:3px 5px;font-size:.66rem;border-left:none}
    #v-outputs .bs.mini.del:hover{background:var(--danger);border-color:var(--danger);color:#fff}
    #v-outputs .jc .sf{flex-wrap:wrap}
  ...
  function render(){
    ...
    // 닫힘은 '0' 으로 표시한다. 예전엔 빈 문자열이라 ||'1' 에 걸려 목표1이 다시 열렸다(접기 불가 버그).
    if(view.dataset.g===undefined || view.dataset.g==='') view.dataset.g='1';
    const openG=view.dataset.g;
  ...
  /* ── 첨부(파일·링크) 조작 버튼 — 기대산출물·세부과제 산출물이 함께 쓴다 ──
   *  올린 뒤에도 교체(🔄)·삭제(✕)가 가능해야 한다. 예전엔 열기만 됐다. */
  function attachBtns(r){
    const s=OUT[r.key]||{}, k=esc(r.key), fn=esc(s.file_name||'');
    const file = s.file_path
      ? '<span class="grp"><button class="bs on" data-act="open" data-k="'+k+'" title="'+(fn||'첨부파일 열기')+'">📎 파일</button>'
        +'<button class="bs mini" data-act="refile" data-k="'+k+'" title="다른 파일로 교체">🔄</button>'
        +'<button class="bs mini del" data-act="delfile" data-k="'+k+'" title="첨부파일 삭제">✕</button></span>'
      : '<button class="bs" data-act="up" data-k="'+k+'">＋파일</button>';
    const link = s.link
      ? '<span class="grp"><button class="bs on" data-act="openlink" data-k="'+k+'" title="'+esc(s.link)+'">🔗 링크</button>'
        +'<button class="bs mini" data-act="link" data-k="'+k+'" title="링크 수정">✎</button>'
        +'<button class="bs mini del" data-act="dellink" data-k="'+k+'" title="링크 삭제">✕</button></span>'
      : '<button class="bs" data-act="link" data-k="'+k+'">＋링크</button>';
    return file+link;
  }
  ...
  function midCard(r){
    ...
    return `<div class="jc"><label class="cl"><input type="checkbox" data-act="done" data-k="${esc(r.key)}"
      ...
      ${numeric?`<div class="num"><input data-act="cur" data-k="${esc(r.key)}" value="${cur}"><em>/</em>
        <input data-act="tg" data-k="${esc(r.key)}" value="${tg}"><em>${esc(unit)}</em><b>${pct}%</b></div>`:''}
      <div class="sf"><div class="acts">${attachBtns(r)}</div></div></div>`;
  }
  ...
  function micCard(r){
    ...
    return `<div class="jc">
      ...
      <div class="sf"><span class="due">${r.due?'~'+esc(r.due):'기한 미정'}</span>
        <div class="acts">${attachBtns(r)}</div></div></div>`;
  }
  ...
  function bind(view){
    view.addEventListener('click',async e=>{
      ...
      if(gh){ const z=gh.parentElement; view.dataset.g = z.classList.contains('open')?'0':z.dataset.g; render(); return; }
      ...
      if(act==='link'){
        if(!_authed()){ toast('🔐 구글 로그인 후 편집할 수 있어요.'); return; }
        const v=prompt('파일서버 경로 또는 URL', s.link||''); if(v===null) return;
        await save(r,{link:v.trim()}); return;
      }
      if(act==='openlink'){
        if(!s.link) return;
        // http(s) 만 새 창으로 연다. 내부 파일서버 경로(\\server\...)는 열 수 없어 복사용으로 안내한다.
        if(/^https?:\/\//i.test(s.link)) window.open(s.link,'_blank','noopener');
        else { prompt('내부 경로입니다. 복사해서 탐색기에 붙여넣으세요.', s.link); }
        return;
      }
      if(act==='dellink'){
        if(!_authed()){ toast('🔐 구글 로그인 후 삭제할 수 있어요.'); return; }
        if(!confirm('링크를 삭제할까요?\n\n'+s.link)) return;
        await save(r,{link:null}); toast('링크를 삭제했어요.'); return;
      }
      if(act==='open'){
      ...
      if(act==='delfile'){
        ...
        if(!confirm('첨부파일을 삭제할까요?\n\n'+(s.file_name||s.file_path))) return;
        const sb=_sb(); if(!sb) return;
        try{
          // 스토리지 실물을 먼저 지우고, 그 다음 참조를 비운다(고아 파일 방지).
          await sb.storage.from(BUCKET).remove([s.file_path]);
          await save(r,{file_path:null, file_name:null});
          toast('첨부파일을 삭제했어요.');
        }catch(err){ toast('삭제 실패: '+(err&&err.message||'')); }
        return;
      }
      if(act==='up' || act==='refile'){
        if(!_authed()){ toast('🔐 구글 로그인 후 첨부할 수 있어요.'); return; }
        const oldPath = (act==='refile') ? s.file_path : null;
        ...
        inp.onchange=async()=>{
          ...
          try{ const {error}=await sb.storage.from(BUCKET).upload(path,f,{upsert:true});
            if(error) throw error;
            await save(r,{file_path:path, file_name:f.name});
            // 교체였다면 이전 파일을 정리한다. 실패해도 본 동작은 이미 성공이므로 무시한다.
            if(oldPath && oldPath!==path){ try{ await sb.storage.from(BUCKET).remove([oldPath]); }catch(_){} }
            toast(act==='refile'?'파일을 교체했어요.':'업로드 완료');
